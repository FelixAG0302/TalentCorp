...
public class EntrevistasController(TalentCorpContext context) : Controller
{
    ...
    {
        var talentCorpContext = context.Entrevistas.Include(e => e.Candidato);
    ...
    public async Task<IActionResult> Create([Bind("Id,CandidatoId,FechaEntrevista")] Entrevista entrevista)
    {
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Edit(int id, [Bind("Id,CandidatoId,FechaEntrevista")] Entrevista entrevista)
    {
        if (id != entrevista.Id)
        {
            return NotFound();
